...
if( $body.hasClass('map-fullscreen') ) {
    if( $(window).width() > 768 ) {
        $('.map-canvas').height( $(window).height() - $('.navbar').height() - $('#map-search-area').height()-50);
    }
    else {
        $('.map-canvas #map').height( $(window).height() - $('.navbar').height() - $('#map-search-area').height()-50);
    }
}
...
function createGoogleMap(_latitude,_longitude, default_zoom){
	$.get("/resource/assets/external/_mobile_infobox2.js", function() {
        gMap();
    });
...
//				document.location.href='/index.php/medical/contents/search_hospital/index?x='+map.getBounds().getCenter().lat()+'&y='+map.getBounds().getCenter().lng()+'&zoom='+map.getZoom();
			...
			setCookie(map.getBounds().getCenter().lat(), map.getBounds().getCenter().lng(), map.getZoom());
...
//			clearData();
			...
			if(data_length){
				json_url = '/m/index.php/mobile/contents/search_hospital/searchLatAdd';
				first_flag = false;
			}

			//json 부분
			var target_json_url = json_url+'?ne='+encodeURI(ne)+'&sw='+encodeURI(sw);
