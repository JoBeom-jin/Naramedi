...
function createHomepageGoogleMap(_latitude,_longitude){
    ...
    function gMap(){		
		...
		*/
		...
		var search_value = $('#ai-name').val();

		if(!search_value){
			$('#row-total').html(json.data.length);
		}
        ...
        markerCluster = new MarkerClusterer(map, newMarkers, { styles: clusterStyles, maxZoom: 17, gridSize:80 });
        ...
        // Dynamic loading markers and data from JSON ----------------------------------------------------------------------
            ...
            $.each(json.data, function(a) {
				...
				if( !search_value){
					...
					$.each(json.data, function(a) {
						if( map.getBounds().contains( new google.maps.LatLng( json.data[a].latitude, json.data[a].longitude ) ) ) {
							is_cached(json.data[a].gallery[0], a, json.data[a].id);
						}
					});
...
//        redrawMap('google', map);
        ...
        function is_cached(src, a, data_id) {
            var image = new Image();
            var loadedImage = $('.results li #' + data_id + ' .image');
            image.src = src;
            if( image.complete ){
                ...
                $(".results").each(function() {
                    $('.results li #' + data_id + ' .image').addClass('loading');
                });
                $(image).load(function(){
                    loadedImage.removeClass('loading');
                    loadedImage.addClass('loaded');
