...
class Customreserveframe extends CI_Controller {
	private $_types = array();
	function __construct(){
		parent::__construct();
		$this->load->model('Hansin/HansinUserModelCustom', 'userModel');
	...
	public function index(){
		...
		$this->load->view('customReserveFrame/hasinmedipia_web', $data);
	}

	function acception(){
		$data = false;
		$this->load->view('customReserveFrame/hasinmedipia_accept', $data);
	}
	...
	function insertOk(){
		...
		if($msg = $this->_checkArgs($args)){
			$data['msg'] = $msg;
			$this->load->view('customReserveFrame/script', $data);
		}else{
