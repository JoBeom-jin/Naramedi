...
class NormalReserve{
	...
	*/
	function doReserveList(&$data, &$ci){
		...
		$paging->addWhere('er_mall','=', 'MLL002');
		$paging->addWhere('er_method','=', 'MED001');

		$csseq = $ci->shopModelGroups->listByMeseq($ci->auth->seq());
		if(isset($csseq[0])){
			$cs_info = $ci->shopModel->getRowBySeq($csseq[0]['csg_csseq']);
			$paging->addWhere('er_mall_name','=', $cs_info['cs_name']);
		}
		else{
			$paging->addWhere('er_mall_name','=', '');
		}
