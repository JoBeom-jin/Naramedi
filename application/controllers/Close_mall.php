...
class Close_mall extends MY_Controller {

	private $CI = false;
	protected $_site_id = 'close_mall';						//선언된 사이트 아이디
	protected $_data = array();

	private $_access_groups = array('CLOSEMALL');

	function __construct() {
		parent::onInit();
		parent::setFrame('close_mall');
	...
	public function index(){
		...
		if($tpl != 'script') $tpl = 'close_mall_view/'.$tpl;
	...
	public function contents(){
		...
		if(!$this->_checkAuth()){
			...
			header('Location: /index.php/close_mall/');
			exit;
		}
	...
	*/
	function doMain(&$data){
		...
		$this->load->model('CloseShop/CloseShopModel', 'shopModel');
		$this->load->model('CloseShop/CloseShopGroupsModel', 'shopModelGroups');

		$data['today_event_reserve'] = 0;
		$csseq = $this->shopModelGroups->listByMeseq($this->auth->seq());
		if(isset($csseq[0])){
			$cs_info = $this->shopModel->getRowBySeq($csseq[0]['csg_csseq']);
			$data['today_event_reserve'] = $this->reserveModel->getTodayEventReserveNumberCloseMall($cs_info['cs_name']);
		}
	...
	*/
	function doLoginOk(&$data){
		...
		// print_r($this->auth->inGroups($this->_access_groups));
		// die();
