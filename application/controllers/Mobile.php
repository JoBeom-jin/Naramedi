...
class Mobile extends MY_Controller {
	...
	private $CI = false;
	protected $_site_id = 'mobile';						//선언된 사이트 아이디
	protected $_data = array();
	private $_is_closed = false;
	private $_closed_seq = false;
	private $_full_url =  false;
	private $_naver_msg = false;

	function __construct() {

        parent::onInit();
		parent::setFrame('mobile');
		parent::addCss('/resource/css/medical_common.css');
		parent::addJs('/resource/js/jquery/jquery-1.12.3.min.js');
		parent::addJs('/resource/assets/js/bootstrap-select.min.js');
		parent::addJs('/resource/assets/js/icheck.min.js');
    ...
    //
		...
		if($closed_seq = $this->CI->request->param('closed_seq', METHOD_BOTH, false)){
			$this->CI->input->set_cookie('closed_seq', $closed_seq, 0);
			$this->_is_closed = true;
			$this->_closed_seq = $closed_seq;
		}

		if($closed_seq = $this->CI->input->cookie('closed_seq')){
			$this->_is_closed = true;
			$this->_closed_seq = $closed_seq;
		}
		...
		$_naver_default_msg = 'OK메디는 휴대폰 기본 브라우저에 최적화되어 있습니다.고객님이 접속하신 네이버앱 브라우저에서는 일부 컨텐츠가 정상적으로 보이지 않을 수 있습니다. OK메디 사이트를 원활하게 이용하기 위해서는 네이버앱 설정메뉴에서 네이버 브라우저엔진(V2)를 사용함으로 설정해 주세요.';

		$data['naver_message'] = false;
		if($this->_naver_check()){
			$this->_naver_msg = $_naver_default_msg;
		}
	}
	...
	public function index(){
		...
		$this->_data['_naver_msg'] = $this->_naver_msg;
	...
	public function contents(){
		...
		//okplus용으로 강제로 상단 지정
		$this_menu = $this->uri->segment(3);
		if( $this_menu == 'search2_hospital2' || $this_menu == 'okmedi2_healthboard2' ){
			$this->setFrame('okplus');
		}
...
//		else if(array_key_exists('hi_revmng_phone', $data['agency'])) $data['phone_number'] = $data['agency']['hi_revmng_phone'];
	...
	private function _naver_check(){
		 if($this->_is_naver_app() && $this->_set_naver_cookie()){
			 return true;
		 }else return false;
	}

	private function _set_naver_cookie(){
		$this->load->helper('cookie');

		$flag_name = 'naver_flag_check';
		if(get_cookie('myprefix_'.$flag_name, TRUE) != '1'){
			$cookie = array(
			   'name'   => $flag_name,
			   'value'  => '1',
			   'expire' => '865000000',
	...
	private function _is_naver_app(){
		$user_agent = $_SERVER['HTTP_USER_AGENT'];
		return preg_match('/naver\(inapp/i', $user_agent);
	}
