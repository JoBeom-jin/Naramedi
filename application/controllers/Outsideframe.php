...
class Outsideframe extends CI_Controller {
	private $_types = array();
	function __construct(){
		parent::__construct();
		$this->load->model('Hansin/HansinUserModel', 'userModel');
	...
	public function index(){
		...
		$this->load->view('outsideFrame/hasinmedipia_web', $data);
	}

	function acception(){
		$data = false;
		$this->load->view('outsideFrame/hasinmedipia_accept', $data);
	}
	...
	function insertOk(){
		...
		if($msg = $this->_checkArgs($args)){
			$data['msg'] = $msg;
			$this->load->view('outsideFrame/script', $data);
		}else{
