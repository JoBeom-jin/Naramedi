<?$add_url = "city=".urlencode($select_city)."&amp;type={$select_type}&amp;hi_seq={$hi_seq}";?>
...
$(document).ready(function(){
	$('.sorting a').on('click', function(e){
		var new_href = $(this).attr('href')+'#'+yeseuledit_scrollTop;
		$(this).attr('href', new_href);
		return true;
	});

	$('#search-opts a').on('click', function(e){
		var new_href = $(this).attr('href')+'#'+yeseuledit_scrollTop;
		$(this).attr('href', new_href);
		return true;
	});
	...
	$(window).scroll(function(){
		yeseuledit_saveScroll();
	});

	var win_scroll = window.location.hash;	
	if(win_scroll){
		win_scroll = parseInt(win_scroll.replace('#', ''));
		$(window).scrollTop(win_scroll);
	}
});

var yeseuledit_scrollTop = 0;
function yeseuledit_saveScroll(){
    yeseuledit_scrollTop = $(window).scrollTop();
}
</script>
