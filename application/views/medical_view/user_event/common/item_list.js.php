...
function makeItem(event_list_href, banner_src, ei_seq, ei_name, is_like, is_close_mall, ei_original_account,  ei_account, ei_discounted_account, end_time, hi_open_name,   address, ei_age_text, ei_theme_text){
	//wrappers
	var list_wrapper = $('<div/>').attr('class', 'col-md-4 col-sm-4');
	...
	var image_link = $('<a/>').attr({
		'href' : event_list_href,
		'style' : 'display:block; height:370px;'
	});
	...
	var splitAddr = address.split(" ", 2);

	var figure = $('<figure/>');
	figure.html(figure.html()+' '+splitAddr[0]+' '+splitAddr[1]+' / '+hi_open_name);
	figure.appendTo(text_wrapper);

	var eName = $('<div/>').attr('class', 'main-item-title').html(ei_name);
	eName.appendTo(text_wrapper);

	var age = $('<div/>').attr('class', 'main-item-desc');
	age.html('<span class=\'item_sub_icon\'> 추천연령 </span> '+ei_age_text);
	age.appendTo(text_wrapper);


	// var type = $('<div/>').attr('class', 'age');
	// type.html('<span> 검진테마 : </span>'+ei_theme_text);
	// type.appendTo(text_wrapper);

	var event_date = $('<div/>').attr('class', 'main-item-desc');
	event_date.html('<span class=\'item_sub_icon\'>이벤트기간</span> '+end_time);
	// $('<span/>').attr('class',' end_time').html(end_time).appendTo(event_date);
	event_date.appendTo(text_wrapper);
	...
	var account = $('<div/>').attr({
		'class':'main-item-price',
		'style':'float:right; color:#195ab3;'
	});
	account.appendTo(text_wrapper);
	...
	if(is_close_mall){
		...
		$('<font/>').attr('style', 'font-size:60%; text-decoration:line-through; color:#676767').html(ei_original_account+' 원').appendTo(account);
