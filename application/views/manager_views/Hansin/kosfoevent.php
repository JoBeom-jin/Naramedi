...
<div id="hansin-users">
	<table class="table table-list" summary="신청자의 이름, 전화 번호등의 정보 목록을 보여줍니다.">
		...
		<tbody>
			...
			<?foreach($list as $k => $v):?>
			<?php if ($v['hu_code'] != 'kosfo' ) { continue;} ?>
			<tr>
			<?
			$hu_type_code = explode('_', $v['hu_type_code']);
			// print_r($hu_type_code);
			?>
				<td class="tcenter"><?=$paging->getPageNum($k)?></td>
				<td class="tcenter"><?=$v['hu_name']?></td>
				<td class="tcenter"><?=$types[$hu_type_code[0]]." / ".$hu_type_code[1]."만원"?></td>
