<div id="agency_manager_view">
	<h2 class="title">병원정보 관리</h2>

	<h3 class="title">병원정보 수정 폼</h3>

	<form method="post" action="<?=$menu_url?>/modifyOk" target="formReceiver">
	<input type="hidden" name="seq" value="<?=$modify['ai_seq']?>" />

	<table class="table table-form" summary="검진기관 수정 폼">
		<caption>병원정보 수정 폼</caption>
		<colgroup>
			<col width="15%" />
			<col width="35%" />
			<col width="15%" />
	...
	<form method="post" action="<?=$menu_url?>/modifyFileOK" enctype="multipart/form-data" target="formReceiver">
			...
			<tbody>
				...
				<tr>
					...
					<td colspan="3">
						...
						<?endforeach;?>
						<?endif;?>										
						<div class="talert" style="padding-top:20px; clear:both;">※ 사진을 클릭하시면 삭제하실 수 있습니다.</div>
					</td>
				</tr>
				<tr>
					<th>이미지등록</th>
		...
		<div class="btn-area-center">
			<input type="submit" value="이미지 수정" class="btn btn-primary"/>
		</div>
	</form>
</div>