				...
				<aside class="sorting">
					<form method="get" action="<?=$menu_url?>/<?=$act?>" id="search-form">
						<div class="form-group">
							<label for="country">이벤트 검색</label>
							<div style="width:30%; float:left; margin-right:3%">
								<select class="framed" name="city" id="city" data-size="10">
									<option value="">시도 선택</option>
									<?foreach($cities as $k => $v):?>
									<option value="<?=$v['ai_addr_text1']?>" <?if($v['ai_addr_text1'] == $select_city):?>selected="selected"<?endif;?>><?=$v['ai_addr_text1']?></option>
									<?endforeach;?>
								</select>
							</div>
							<div style="width:30%; float:left; margin-right:3%">
								<select class="framed" name="local" id="local" data-size="10">
									<option value="">시군구 선택</option>
									<?if(is_array($local) && count($local) > 0):?>
									<?foreach($local as $k => $v):?>
									<option value="<?=$v['ai_addr_text2']?>" <?if($v['ai_addr_text2'] == $select_local):?>selected="selected"<?endif;?>><?=$v['ai_addr_text2']?></option>
									<?endforeach;?>
									<?endif;?>
								</select>
							</div>
							<div style="width:30%; float:left; margin-right:3%">
								<select class="framed" name="type" id="type" data-size="10">
									<option value="">유형 선택</option>
									<?foreach($hos_types as $v):?>
									<option value="<?=$v['cd_code']?>" <?if($select_type == $v['cd_code']):?>selected="selected"<?endif;?>><?=$v['cd_name']?></option>
									<?endforeach;?>
								</select>
							</div>
						</div>
