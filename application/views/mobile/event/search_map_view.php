<div id="search-map_view">
	<!-- gallery list -->
	<div class="gallery">
		<div class="image">
			<div class="owl-carousel gallery">
			<!-- 병원 클릭후 해당병원 외관 이미지 DB -->
	...
	<div class="row card card-1" style="width:100%; margin:0; margin-top:20px; ">
		<div>
			<h2><?=$agency['ai_name']?></h2>
			<h4><?=$agency['ai_addr']?></h4>

			<p class="hospital-icons">
				<?
					//출력할 아이콘 리스트
					$icon_list = array();
					if($is_member) {$icon_list[] = '<img src="/resource/assets/img/i_co.png">';}
					if($map_url) $icon_list[] = '<a href="'.$map_url.'" target="_blank"><img src="/resource/assets/img/i_road.png" alt="길찾기"></a>';
					if($has_event) $icon_list[] = '<a href="/m/index.php/mobile/contents/event_hospital?hi_seq='.$is_member.'"><img src="/resource/assets/img/i_event.png"></a>';
		...
		<div class="tabbable-line" >
			<ul class="nav nav-tabs">
				<li id="tab-1" class="active">
					<a href="#tab_default_1" class="noact" data-toggle="tab"><font size=4 >검진기관정보 </font></a>
				</li>
				<li id="tab-2">
					<a href="#tab_default_2" class="noact" data-toggle="tab" id="tab-reply"><font size=4>평점 및 후기 </font></a>
				</li>
			</ul>
			...
			<div class="tab-content">
				...
				<div class="tab-pane active" id="tab_default_1">
					<article class="block"  style="margin:10px 0px; padding:0px 15px;">
						<header><h2><img src="/resource/images/medical/icons/clock.png" style="">&nbsp; 검진 시간</h2></header>
					...
					<article class="block" style="margin:10px 0px; padding:0px 15px;">
						<header><h2><img src="/resource/images/medical/icons/dod.png" >&nbsp; 검진항목</h2></header>
							<div  style="margin:10px 0px 0px 30px;">
								<?if($agency['ai_gren_cd']):?>
								<img src="/resource/images/medical/icons/icon_gen.png" style="height:100px; margin-bottom:5px;">
								<?endif;?>

								<?if($agency['ai_stmca_cd']):?>
								<img src="/resource/images/medical/icons/icon_we.png" style="height:100px; margin-bottom:5px;">
								<?endif;?>


								<?if($agency['ai_cc_cd']):?>
								<img src="/resource/images/medical/icons/icon_deajang.png" style="height:100px; margin-bottom:5px;">
								<?endif;?>


								<?if($agency['ai_lvca_cd']):?>
								<img src="/resource/images/medical/icons/icon_gan.png" style="height:100px; margin-bottom:5px;">
								<?endif;?>

								<?if($agency['ai_bc_cd']):?>
								<img src="/resource/images/medical/icons/icon_chest.png" style="height:100px; margin-bottom:5px;">
								<?endif;?>

								<?if($agency['ai_cvxca_cd']):?>
								<img src="/resource/images/medical/icons/icon_jagung.png" style="height:100px; margin-bottom:5px;">
								<?endif;?>


								<?if($agency['ai_mchk_cd']):?>
								<img src="/resource/images/medical/icons/icon_mouth.png" style="height:100px; margin-bottom:5px;">
								<?endif;?>					
					...
					<article class="block"  style="margin:10px 0px; padding:0px 15px;">
							<header><h2><img src="/resource/images/medical/icons/car.png" >&nbsp; 교통 및 주차정보</h2></header>
							<dl class="lines" style="margin:10px 30px 0px 30px;">
								<dt>교통</dt>
								<?if($agency['at_inc_bus']):?>
								<dd>시내버스 : <?=$agency['at_inc_bus']?> / <?=$agency['at_inc_bus_goal']?> / <?=$agency['at_inc_bus_way']?>
								<?if(is_numeric($agency['at_inc_bus_dis'])):?> / <?=number_format($agency['at_inc_bus_dis'])?>미터<?endif;?>
								<?else:?>
								<dd>시내 버스 : 정보없음</dd>
								<?endif;?>
					...
					<article class="block"  style="margin:10px 0px; padding:0px 15px;">
							<header><h2><img src="/resource/images/medical/icons/go.png" >&nbsp; 위치</h2></header>
					</article>
				...
				<div class="tab-pane" id="tab_default_2">
					...
					<section class="block" id="reviews">
						<header class="clearfix">
							...
							<a href="#write-review" class="btn btn-danger icon pull-right roll">후기 등록<i class="fa fa-pencil"></i></a>
						</header>
						...
						<section class="reviews">
							...
							<article class="review">
								...
								<div class="wrapper">
									...
									<p style="width:100%; margin:0; clear:both;">
										<?=nl2br(strip_tags(trim($v['ac_comment'])));?>
									</p>
								</div>
	...
	<table id="service-btn-area" style="width:100%; margin:0 auto; left:0;">
		<?if($is_member):?>
		<colgroup>
			<col width="50%" />
			<col width="50%" />
		...
		<tr>
			<td class="left-btn">
				<a href="/m/index.php/mobile/contents/event_hospital?hi_seq=<?=$is_member?>">간편예약</a>
			</td>
			<td class="right-btn">
				<a href="tel:<?=$phone_number?>" data-popnum="<?=$agency['ai_seq']?>">전화상담</a>
			</td>		
		</tr>
		<?else:?>
		<colgroup>
...
<style type="text/css">
.simple-map{width:100%; margin:0 auto; height:400px; border:1px solid #e3e3e3;}
...
#service-btn-area .left-btn{height:100%; background-color:#393f4b; font-size:20px; color:#fff; font-weight:bold; text-align:center;}
#service-btn-area .left-btn a{color:#fff;}
#service-btn-area .right-btn{height:100%; background-color:#4e94fe; font-size:20px; color:#fff; font-weight:bold; text-align:center;}
#service-btn-area .right-btn a{color:#fff;}
...
function checkOffset() {
    if($('#service-btn-area').offset().top + $('#service-btn-area').height() 
                                           >= $('footer').offset().top - 10)
        $('#service-btn-area').css('position', 'absolute');
    if($(document).scrollTop() + window.innerHeight < $('footer').offset().top)
        $('#service-btn-area').css('position', 'fixed'); // restore when you scroll up
}
</script>